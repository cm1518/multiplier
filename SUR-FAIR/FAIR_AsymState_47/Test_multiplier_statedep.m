...
INDICATOR=[prctile(setup.indicator(1:end),20),prctile(setup.indicator(1:end),80)]
...
display(['std:     ']);
display(num2str([INDICATOR' squeeze(std(Msum,0,1))']))
...
%Msum_pos
figure, scatter(squeeze(Msum(:,2,1)),squeeze(Msum(:,2,2)),5)
hold on, line([-.5 5],[-.5 5],'LineStyle','--','Color','r')
xlim([-0.2 3]),xlabel('m^{+,U low}_{sum}','FontName','cmr10','FontSize',12)
ylim([-0.2 3]),ylabel('m^{+,U high}_{sum}','FontName','cmr10','FontSize',12)
...
%Msum_neg
figure, scatter(squeeze(Msum(:,1,1)),squeeze(Msum(:,1,2)),5)
hold on, line([-.5 5],[-.5 5],'LineStyle','--','Color','r')
xlim([-0.2 5]),xlabel('m^{-,U low}_{sum}')
ylim([-0.2 5]),ylabel('m^{-,U high}_{sum}')
...
%confidence intervals for m_high and m_low
for sgn=1:2
    for index=1:2
        
        sgn
        index
        [prctile(Msum(:,sgn,index),5) prctile(Msum(:,sgn,index),50) prctile(Msum(:,sgn,index),95)]
    end
end
