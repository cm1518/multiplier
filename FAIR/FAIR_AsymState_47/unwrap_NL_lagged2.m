function [ Sigma] = unwrap_NL_lagged2(  alpha_gen_neg,beta_gen_neg,b_gen_neg,c_gen_neg,alpha_gen_pos,beta_gen_pos,b_gen_pos,c_gen_pos,epsilon_vec,setup ,indicator_vec,ind_mean)
...
for jj=1:setup.lags
   ...
   Sigma(:,setup.index_unrestricted,jj+1)=sum(SL_NL_3( alpha_gen_neg(:,:),beta_gen_neg(:),b_gen_neg(:,:),c_gen_neg(:,:),alpha_gen_pos(:,:),beta_gen_pos(:),b_gen_pos(:,:),c_gen_pos(:,:),epsilon_vec(setup.index_unrestricted,jj+1),jj,setup.size_obs,indicator_vec(end-jj+1),ind_mean ),2);
